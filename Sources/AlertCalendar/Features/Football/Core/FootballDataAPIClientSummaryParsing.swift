...
extension FootballDataAPIClient {
    ...
    static func liveMatch(
        ...
        isNationalCompetition: Bool? = nil
        ...
        let isNationalCompetition = isNationalCompetition ?? (
            FootballCompetitionPreset.category(forCompetitionSlug: competitionSlug) == .nationalTeams
        )
        let homeName = teamName(from: homeTeam)
        let awayName = teamName(from: awayTeam)
        let homeAbbreviation = teamAbbreviation(from: homeTeam, fallbackName: homeName)
        let awayAbbreviation = teamAbbreviation(from: awayTeam, fallbackName: awayName)
        let homeSummary = FootballTeamSummary(
            ...
            logoURL: FootballFederationLogoResolver.resolvedLogoURL(
                ...
                abbreviation: homeAbbreviation,
                countryName: nil,
                isNational: isNationalCompetition
            ),
            countryName: nil,
            isNational: isNationalCompetition
        )
